...
export const usePracticesStore = defineStore('practices', {
  ...
  actions: {
    ...
    addNewPractice(newPractice) {
      ...
      newPractice.id = Object.values(this.practices).map(practiceType => practiceType.list).flat().length + 1;
    ...
    getPracticeById(id) {
      return Object.values(this.practices).map(practiceType => practiceType.list).flat().find(practice => practice.id === id);
    }
  },
});
